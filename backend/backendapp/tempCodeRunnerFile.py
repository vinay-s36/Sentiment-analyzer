...
def inputfunction(X):
    with open('D:\\OneDrive\\Desktop\\knn_model.pkl','rb') as model_file:
        loaded_knn_model=pickle.load(model_file)
    y=calculate_sentiment(X)
    l=len(X.split())
    X=[X]
    
    cv=TfidfVectorizer(norm='l2',analyzer='word',ngram_range=(1,3),dtype=np.float64)
